...
export default function Edit({ model }) {
    ...
    function handleSubmit(e) {
        e.preventDefault();
        put(route('paymentmethods.update', model.id));
    }
    ...
    return (
        <>
          <AuthenticatedLayout
            user={auth.user}
            header={<h2 className="font-semibold text-xl text-light-800 leading-tight">Edit PaymentMethod</h2>}
        >
            <h1 className="text-2xl font-bold">Edit PaymentMethod</h1>
            <hr className="my-4" />
            <form onSubmit={handleSubmit}>
                <Form data={data} setData={setData} errors={errors} />
                <div className="mt-4">
                    {auth.user.permissions.includes('paymentmethods_update') && (
