...
export default function BatchIndex({ auth, batches }) {
    ...
    return (
        <AuthenticatedLayout
            user={auth.user}
            header={<h2 className="font-semibold text-xl text-light-800 leading-tight">Batches</h2>}
        >
            <Head title="Batches" />
                                ...
                                <table className="min-w-full divide-y divide-gray-200">
                                    ...
                                    <tbody className="bg-white divide-y divide-gray-200">
                                        {batches.length > 0 ? (
                                            batches.map((batch) => (
                                                <tr key={batch.id}>
                                                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{batch.course.name}</td>
                                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{batch.name}</td>
                                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{batch.start_time}</td>
                                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{batch.end_time}</td>
                                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{batch.status}</td>
                                            ...
                                            <tr>
                                                <td colSpan="6" className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center">No batches found.</td>
                                            </tr>
                                        )}
                                    </tbody>
                                </table>
